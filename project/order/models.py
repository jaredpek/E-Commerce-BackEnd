...
class Order(models.Model):
    ...
    items_subtotal = models.FloatField(validators=[MinValueValidator(0)])
    delivery_cost = models.FloatField(validators=[MinValueValidator(0)])
...
class OrderItem(models.Model):
    ...
    def __str__(self):
        return f'{self.user.username} | {self.restaurant.name} | {self.item.name} | {self.quantity} | {self.total_cost}'

...
class OrderDetails(generics.ListCreateAPIView):
    ...
    def getLocation(self, postal_code, key):
    ...
    def getDistance(self, start, end, key):
        ...
        if create_order_serializer.is_valid(raise_exception=True):
            ...
            if address_serializer.is_valid(raise_exception=True):
                ...
                distance = self.getDistance(
                    start=self.getLocation(restaurant.postal_code, BING_APIKEY),
                    end=self.getLocation(deliver_to['to_postal_code'], BING_APIKEY),
                    key=BING_APIKEY,
                )
